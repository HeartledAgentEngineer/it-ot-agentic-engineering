...
|---|---|---|---|---|
| **`eu`** (Standard) | Reine OpenRouter-Kette: `mai-transcribe-2` → `mai-transcribe-1.5` → `voxtral-…-stt` → Voxtral Small (Audio-Chat) | 3,8 s bei 140 s Audio | 325 von 325 Wörtern ✅ | 0,10 $ |
...
**Modellwahl nach Benchmark und eigener Messung (25.09.2026):** Für Deutsch liegt `microsoft/mai-transcribe-2` vorn — an 140 s natürlichem Deutsch **0,3 % Wortfehler in 3,8 s** (325 von 325 Wörtern), Kosten 0,10 $/Stunde. Zum Vergleich im selben Lauf: `mai-transcribe-1.5` 0,3 % in 11,8 s (0,36 $/h), `voxtral-…-stt` 0,3 % in 18,9 s, `whisper-large-v3` 1,2 % in 4,5 s, der Voxtral-Chat-Weg lief in eine 429. Deshalb steht `mai-transcribe-2` an erster Stelle der Standardkette; die anderen rücken nach, wenn er nicht liefern kann. Alle Wege sind über OpenRouter erreichbar und erfüllen die ZDR-Einstellung des Kontos: OpenRouter lehnt nicht konforme Endpunkte ab (`404 ZDR violation` bzw. `0 endpoints out of 1 requested`) — was liefert, ist damit nachweislich konform. `google/gemini-3.5-transcribe` fällt genau daran aus. ElevenLabs Scribe v2 wäre laut Benchmark gleichauf (2,2 %), erfüllt die ZDR-Vorgabe aber nicht: echtes Zero-Retention ist dort Enterprise-Kunden vorbehalten, und über OpenRouter ist es nicht erreichbar.

**Drosselung entschärft:** Der Audio-Chat-Weg läuft bei Mistral im *geteilten* Anbieter-Pool und bekam ab etwa 60 s Audio durchgehend `429 rate-limited upstream` — lange Diktate endeten im roten Symbol. In der Kette stehen deshalb jetzt die **Transkriptions-Endpunkt-Wege vorn**; sie kennen diese Drosselung nicht (69,7 s in 4,8 s, 139 s im Test). Der Chat-Weg ist nur noch das dritte Glied und liefert weiterhin als einziger die Fachbegriffe mit. Zweiter Befund aus dem Betrieb: Voxtral gab einmal den Auftragstext selbst zurück; solche Antworten verwirft der Code inzwischen (`_ist_auftragstext`).
...
* **Zweistufige Sprachverarbeitung mit wählbarem Weg:** Die Transkription läuft über eine **Anbieterkette**, die in der `config.json` gewählt wird — `eu` (Standard: reine OpenRouter-Kette — `microsoft/mai-transcribe-1.5` und `mistralai/voxtral-small-24b-2507-stt` über den Transkriptions-Endpunkt, danach Voxtral Small im Audio-Chat), `beste` (ElevenLabs Scribe v2) oder `schnell` (Groq `whisper-large-v3`). Jeder Anbieter wird nur versucht, wenn sein Schlüssel in der `.env` steht; fehlt der Schlüssel des gewählten Wegs, nimmt typeFREE den anderen und protokolliert das; kann keiner liefern, meldet typeFREE einen Fehler, statt still zu schweigen. Jeder Aufruf geht mit `language="de"` und einem **Vokabel-Hinweis** — es gibt zwei Listen, weil die Wege verschiedene Grenzen haben: die **volle Fachliste** (`FACH_VOKABULAR`, 98 Begriffe: Denglisch, IT-Fachsprache, Azure-Kurs, Werkzeuge) geht an ElevenLabs als `keyterms` (bis zu 100 Begriffe — die Liste bleibt darunter, sonst fiele der Rest still weg) und in den Auftragstext des Chat-Wegs; der **kurze Kern** (`WHISPER_VOKABULAR`, 51 Begriffe) geht als `prompt` an Whisper, weil dort eine **harte Grenze von 224 Tokens** gilt und längere Hinweise von der API still gekürzt werden (Groq-Doku: „max 224 tokens"; OpenAI-Cookbook: nur die letzten 224 Tokens zählen). Gemessen am 25.09.2026: 147 Tokens für den Kern (cl100k_base, tiktoken 0.12), inklusive Sicherheitszuschlag für den mehrsprachigen Whisper-Tokenizer ~198 von 224 — ein Test hält die Liste in dieser Grenze. „Commit" und „Comet" stehen in beiden Listen: der belegte Verhörer vom 25.09.2026. Anschließend glättet OpenRouter den Rohtext über eine **Modellkette** (`google/gemini-2.5-flash`, Ausweichwege `google/gemini-3.5-flash-lite` und `google/gemini-2.5-flash-lite`): Füllwörter raus (auch großgeschriebene wie „ÄHM"), Verhaspler geglättet, **offensichtliche Verhörer aus dem Zusammenhang korrigiert** — Umgangssprache und Slang bleiben dabei ausdrücklich unangetastet. Ist das Ergebnis auffällig kürzer als der Rohtext (abgeschnitten oder das Modell hat geantwortet statt bereinigt), wird der Rohtext eingefügt. **Anrede und Sprechakt bleiben unangetastet:** „du" bleibt „du", „Sie" bleibt „Sie", eine Aussage wird keine Frage und eine Bitte keine Feststellung — kein Erzähl- und kein Fragestil. Fällt die Glättung dreimal in Folge aus, erscheint einmalig ein Windows-Hinweis — damit ein abgekündigtes Modell den Filter nicht wieder wochenlang stilllegt.
* **Aussteuerung im Log:** Zu jedem Diktat schreibt typeFREE den Pegel („Aussteuerung: Spitze 0,64 · RMS 0,088") und warnt unter RMS 0,02 — denn ein Diktat kann nur so gut sein wie die Aufnahme. Referenzmessung vom 25.09.2026: sauberes deutsches Referenzaudio (`windows/referenzaudio/de_referenz.wav`, 69,7 s, Wortlaut im Repo) kommt auf **4,0 % Wortfehler**, dieselbe Passage in 25-Sekunden-Happen auf 3,4 % — Länge und Happen machen praktisch keinen Unterschied. Schnelles Sprechen (+30 %), leises Sprechen (−35 %) und viele Füllwörter kosteten 0,0 bis 1,2 %. Damit ist belegt: sinnlose Wörter am Ende eines Diktats („Brother 1", „Buster Brauch" im Betriebslog) kommen aus der Aufnahme an dieser Stelle, nicht aus dem Modell. Nachmessen: `py -3.12 windows/sprachmessung.py -d windows/referenzaudio/de_referenz.wav --happen 25` — der Messweg wurde bewusst **nicht** in die App eingebaut, weil er nichts bringt.
* **Kein Diktat geht verloren:** Schlägt die Transkription fehl (Drosselung, Netz), bleibt die Aufnahme im Arbeitsspeicher, und das Tray-Menü bietet **„Letztes Diktat erneut versuchen (N s)"** an — ein Klick statt neu sprechen. Die Aufnahme liegt weiterhin nur im RAM (kein Festplatten-I/O), und sobald ein Versuch angekommen ist, wird sie verworfen. Anlass: Am 25.09.2026 gingen mehrere lange Diktate durch die Drosselung des Anbieters verloren und mussten neu gesprochen werden.
...
* **Mitlaufende Kostenrechnung:** Das Tray-Menü zeigt Minuten und Betrag für den laufenden Monat und insgesamt. Gebucht wird **beim Transkribieren** mit dem Preis des Anbieters, der tatsächlich geliefert hat — Groq `whisper-large-v3` $0,111 je Stunde, Voxtral (Mistral über OpenRouter) rund $0,36 je Stunde (aus der Anbieter-Abrechnung ermittelt), ElevenLabs Scribe v2 $0,264 je Stunde (0,22 $ plus 20 % Keyterms-Aufschlag). Die Beträge sind damit die **Summe der tatsächlich gebuchten Diktate**; laufen im Monat mehrere Wege, nennt die Anzeige den ersten mit „+" („(Groq +)"). Vorher wurde die Monatssumme mit dem Preis des gerade gewählten Wegs umgerechnet — im Betrieb am 25.09.2026 wurden aus 0,33 $ dadurch 1,04 $, ohne dass ein Cent mehr ausgegeben war. Die Audiolänge ist im Programm exakt bekannt — die Kosten lassen sich also ohne Zusatzabfrage und ohne zweiten Zugangsschlüssel mitrechnen. Gebucht wird erst nach erfolgreichem Einfügen: für fehlgeschlagene Anfragen erscheint kein Betrag. Den exakten Preis jedes einzelnen Diktats schreibt typeFREE in die Logdatei.
...
### Warum alles über OpenRouter läuft
