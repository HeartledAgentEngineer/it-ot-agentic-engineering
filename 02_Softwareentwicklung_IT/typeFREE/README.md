...
|---|---|---|---|---|
| **`eu`** (Standard) | Reine OpenRouter-Kette: `mai-transcribe-1.5` → `voxtral-small-24b-2507-stt` → Voxtral Small (Audio-Chat) | 4,8 s bei 69,7 s Audio | vollständig ✅ | 0,36 $ |
...
**Modellwahl nach Benchmark und eigener Messung (25.09.2026):** Für Deutsch liegt `microsoft/mai-transcribe-1.5` vorn — laut Artificial-Analysis-Bestenliste 2,4 % Wortfehler und bei multilingualen Fleurs-Aufgaben mit 4,86 % der beste veröffentlichte Wert, in der eigenen Messung an 69,7 s Referenzaudio **1,1 %** (Whisper large-v3: 3,4 %). Es ist über OpenRouter erreichbar und erfüllt die ZDR-Einstellung des Kontos: OpenRouter lehnt nicht konforme Endpunkte mit `404 ZDR violation` ab — dieser liefert. Deshalb steht es an erster Stelle der Standardkette. ElevenLabs Scribe v2 wäre laut Benchmark gleichauf (2,2 %), erfüllt die ZDR-Vorgabe aber nicht: echtes Zero-Retention ist dort Enterprise-Kunden vorbehalten, und über OpenRouter ist es nicht erreichbar.
