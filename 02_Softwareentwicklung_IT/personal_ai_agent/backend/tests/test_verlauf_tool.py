...
def test_verlauf_findet_treffer():
    ...
    ausgabe = _verlauf_tool("Was haben wir zu Azure gesagt?", verlauf=nachrichten)
...
def test_verlauf_kein_signal_leer():
    ...
    assert _verlauf_tool("Wie hübsch ist Hamburg?", verlauf={}) == ""


def test_verlauf_kein_treffer():
    """Signal + kein Treffer → Hinweis statt Absturz."""
    ausgabe = _verlauf_tool("Was haben wir zu unsichtbar gesagt?", verlauf={"conv1": []})
